  ...
  monitor: function(upid) {
    if(!this.periodicExecuter) {
      this.periodicExecuter = new PeriodicalExecuter(function() {
        ...
        new Ajax.Request('/audiography/progress?upload_id=' + upid);
      }, 3);
    }
   ...
   finish: function() {
     ...
     $('results').innerHTML = "Upload complete. <a href=''>Have a look</a>.";
   },
   ...
   FileField: {
     add: function() {
       new Insertion.Bottom('file-fields', '<p style="display:none"><input id="file_upload_file" name="file_upload[file]" type="file" /> <a href="#" onclick="UploadProgress.FileField.remove(this);return false;">x</a></p>')
